...
nombreCorrida     = 'DFIT_DFN_15degreesNoRefPropagacionBuffPerm'; % Nombre de la corrida. La corrida se guarda en la carpeta "Resultado de corridas" en una subcarpeta con este nombre.

cargaDatos     = 'load'; % Forma en la que se cargan las propiedades de entrada. "load" "test" "default" "change".
archivoLectura = 'DFITtripleEncuentro_rev052022_062023.txt';%'DFIT_rev052022_WI+DFN_062023.txt';%'DFIT_rev052022_WI062023CorridaCorta.txt';%'DFIT_rev052022_WI+DFN062023CorridaCorta.txt';%'Dfit_rev052022_DFIT_062023.txt'; %'Dfit_rev052022_DFIT_WItrial_062023.txt';% Nombre del archivo con las propiedades de entrada. 
...
direccionRestart   = 'D:\Geomec\paper DFN\ITBA\Piloto\DFIT\Resultados de corridas (.mat)\DFIT_DFN_15degreesNoRefPropagacion\';
propiedadesRestart = 'resultadosFinFractura_DFIT_DFN_15degreesNoRefPropagacion.mat';
...
%-------------------------------------------------------------------------%
% -- Fluidos.
meshInfo.CRFluidos = [meshInfo.CRFluidos zeros(size(meshInfo.CRFluidos,1),3)];
meshInfo.CRFluidos = [meshInfo.CRFluidos ; find(nodTripleEncuentro)'];
    ...
    while error == noConvergido  
        ...
        for iEle = aux(meshInfo.elementsFluidos.activos)
            if any(iEle == propanteProperties.propantesActivosTotales)
                ...
                if any(iEle == elCohesiveElementID)
                    factor = factorPermBuff;
                else
                    factor = 1;
                end
...
%             errorRelCohesivos{iTime}(1,nIter) = norm(dN - dNPrevITER_Error);
    ...
    if ismember(find(nodTripleEncuentro),nodosMuertos)
        disp('LCDTMAllBoys')
    end
