...
initialSrainSolPG = cell(paramDiscEle.nel,1);
for iEle = 1:paramDiscEle.nel
    ...
    BiotE       = Biot{iEle};
    constitutive = constitutivas{iEle};
    if any(iEle == meshInfo.elementsBarreras.index)
        initialSressExtPG = initialSressExtL;
    else
        initialSressExtPG = initialSressExtS;
%         stress2strain = constitutive(:,:,1)\(initialSressExtS+BiotE(:,:,1)*initialPPoral);
%         initialStrainExtS(3) = stress2strain(3);
        initialStrainExtPG = initialStrainExtS;
    end
...
%         initialStressSolPG{iEle}(:,ipg) = initialSressExtPG + BiotE(:,:,ipg)*initialPPoral;
        initialSrainSolPG{iEle}(:,ipg) = initialStrainExtPG;% - constitutive(:,:,ipg)\BiotE(:,:,ipg)*initialPPoral;
%         Re = Re - B'*initialStressSolPG{iEle}(:,ipg)*pGaussParam.wpg(ipg)*det(jac);   
        Re = Re - B'*constitutive(:,:,ipg)*initialSrainSolPG{iEle}(:,ipg)*pGaussParam.wpg(ipg)*det(jac);   

        eleTestigo = string(iEle);
        assert(det(jac)>0,eleTestigo)
    end
