...
%%
%iTime=XX; %para verla en el timestep que quiera
...
if exist('plotdNOdS','var') == 0
    plotdNOdS=0; %0: plot dN - 1: plot dS
end


disp('NO HAY SCATTER NODOS MUERTOS')
%nodos muertos por timestep para el scatter3
nodosMuertos             = reshape(meshInfo.elementsBarra.ALL(unique(meshInfo.cohesivos.relatedEB(logical(meshInfo.cohesivos.deadFlagTimes(:,:,iTime-1)))),:),[],1);
...
if plot12==0
    ...
    if plotConcentracionPropante==0
        if bandplotOPlotColo==0
            ...
            elseif plotdNOdS==1
                %para plotear dS:
                bandplot(meshInfo.cohesivos.elements,meshInfo.nodes,sqrt(meshInfo.cohesivos.dS1Times(:,:,iTime).^2+meshInfo.cohesivos.dS2Times(:,:,iTime).^2))
            end
        else
        ...
        elseif plotdNOdS==1
            %para plotear dS:
            title({nombreCaso,['Corte (dS) de fractura - iTime: ',num2str(iTime)]})
        end
    else
...
%dITER(paramDiscEle.nDofTot_U+1:paramDiscEle.nDofTot_U+paramDiscEle.nDofTot_P)*temporalProperties.preCond;
