...
direccionRestart   = 'D:\Geomec\paper DFN\ITBA\Piloto\DFIT\Resultados de corridas (.mat)\DFIT_tripleEncuentro\';
propiedadesRestart = 'resultadosFinFractura_DFIT_tripleEncuentro.mat';
...
while algorithmProperties.elapsedTime <= temporalProperties.tiempoTotalCorrida
    ...
    elseif isipKC && algorithmProperties.elapsedTime >= temporalProperties.tInicioISIP % Se establece el valor de permeabilidad mas elevado para el SRV que se activa durante la produccion.
        ...
        if KeyInicioIsip
            ...
            I_dt = tensionHidroDrainTimes/3;
            I_ISIP = tensionHidroInicioIsip/3;
            DeltaPHidro = abs(abs(I_ISIP)-abs(I_dt));
%             DeltaPHidro=abs(abs(tensionHidroDrainTimes(Elem))-abs(tensionHidroInicioIsip(Elem))); %% diferencia de tensionesH entre el itime 2 y donde arranca el Isip
        ...
        calcTensionesenISIP
        % calculo los invariantes
        I_itime = tensionHidroIsip/3;
        
        if wantBuffPermeability
            ImproveFactor=permFactor+(1-permFactor/DeltaPHidro)*(I_dt-I_itime').*((I_dt-I_itime')>0);
