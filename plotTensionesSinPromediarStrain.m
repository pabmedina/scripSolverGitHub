...
strain = zeros(paramDiscEle.nel,paramDiscEle.nNodEl,6);
for itime = iTimeEspecifico
    for iele = 1:paramDiscEle.nel
        ...
        for npg = 1:size(unod,1)
            ...
            stressPG(iele,npg,:) = (constitutivas{iele}(:,:,npg))*B*dTimes(eleDofs,itime) + (constitutivas{iele}(:,:,npg))*initialSrainSolPG{iele}(:,npg);
            stressInit = (constitutivas{iele}(:,:,npg))*initialSrainSolPG{iele}(:,npg);
            strain(iele,npg,:) = B*dTimes(eleDofs,itime) + initialSrainSolPG{iele}(:,npg);
...
strainPorePressure = zeros(paramDiscEle.nel,paramDiscEle.nNodEl,6);
for iTime = iTimeEspecifico
    ...
    for iele = 1:paramDiscEle.nel
        for nP = 1:size(unod,1)
            ...
            strainPorePressure(iele,nP,:) = constitutivas{iele}(:,:,nP)\ p;
        end
    end
end


tensionEfectivaPG = stressPG - presionEfectivaPG; %para ver tensiones totales
% tensionEfectivaPG = stressPG; %para ver tensiones efectivas en la roca
strainEfectiva = strain;
...
strainNodal = zeros(paramDiscEle.nel,8,6) ;
...
for iele = 1:paramDiscEle.nel
    for ipg = 1:size(unodNodal,1)
        ...
        strainNodal(iele,ipg,1) = N*strainEfectiva(iele,:,1)';
        strainNodal(iele,ipg,2) = N*strainEfectiva(iele,:,2)';
        strainNodal(iele,ipg,3) = N*strainEfectiva(iele,:,3)';
        strainNodal(iele,ipg,4) = N*strainEfectiva(iele,:,4)';
        strainNodal(iele,ipg,5) = N*strainEfectiva(iele,:,5)';
        strainNodal(iele,ipg,6) = N*strainEfectiva(iele,:,6)';
    end
end
...
% plotElemental(meshInfo.nodes,meshInfo.elements,presionEfectivaPG(:,:,1),'Y','Tensiones Sin Promediar','x [mm]','y [mm]','z [mm]','px [psi]')
% plotElemental(meshInfo.nodes,meshInfo.elements,presionEfectivaPG(:,:,2),'Y','Tensiones Sin Promediar','x [mm]','y [mm]','z [mm]','py [psi]')
% plotElemental(meshInfo.nodes,meshInfo.elements,presionEfectivaPG(:,:,3),'Y','Tensiones Sin Promediar','x [mm]','y [mm]','z [mm]','pz [psi]')
% plotElemental(meshInfo.nodes,meshInfo.elements,presionEfectivaPG(:,:,4),'Y','Tensiones Sin Promediar','x [mm]','y [mm]','z [mm]','pxy [psi]')
% 

if plotStrain
    plotElemental(meshInfo.nodes,meshInfo.elements,strainEfectiva(:,:,1),'Y','Tensiones Sin Promediar','x [mm]','y [mm]','z [mm]','Sx [psi]')
    plotElemental(meshInfo.nodes,meshInfo.elements,strainEfectiva(:,:,2),'Y','Tensiones Sin Promediar','x [mm]','y [mm]','z [mm]','Sy [psi]')
    plotElemental(meshInfo.nodes,meshInfo.elements,strainEfectiva(:,:,3),'Y','Tensiones Sin Promediar','x [mm]','y [mm]','z [mm]','Sz [psi]')
end
